...
visualising multiplexed spatial biology data from Akoya Biosciences
platforms (PhenoCycler, CODEX, PhenoImager).

A typical workflow involves:

1. **Reading** cell segmentation data with `read_akoya()`
...
Since real Akoya data files can be large, we will create a small
...
```{r read-data}
cells <- read_akoya(tmp, sample_id = "example")
head(cells)
```
